...
predict = np.logical_or(rscale_predict > 0.5, rslice_predict > 0.5)

# Display
viewer = napari.Viewer()
viewer.add_image(np.stack(rscale)) 
viewer.add_image(
    np.stack(rscale_predict > 0.5), 
    name="rscale_predict", colormap="green", blending="additive",
    ) 
viewer.add_image(
    np.stack(rslice_predict > 0.5), 
    name="rslice_predict", colormap="magenta", blending="additive",
    ) 
viewer.add_image(
    np.stack(predict > 0.5), name="predict", blending="additive",
    ) 
